...
const App: React.FC = () => {
  ...
  const [history, setHistory] = useState<
    Array<{ command: string; output: string; status: 'success' | 'error' }>
  >([])
  const [canAdvance, setCanAdvance] = useState(false)
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)
  const [mobileTab, setMobileTab] = useState<'terminal' | 'visualizer'>(
    'terminal'
  )
    ...
    if (history.length > 0) {
      ...
      if (lastEntry.status === 'success') {
        ...
        if (isExactMatch) {
          ...
          step.command.startsWith('SET') &&
          expectedCmd.includes('user:name') &&
          keyMatches('user:name', 'Alice')
        ) {
          setCanAdvance(true)
        } else if (
          step.command.startsWith('DEL') &&
          expectedCmd.includes('user:name') &&
          keyDeleted('user:name')
        ) {
          setCanAdvance(true)
        } else if (step.command.startsWith('LPUSH') && keyExists('todo_list')) {
          if (store['todo_list'].value[0] === 'urgent_task') setCanAdvance(true)
        } else if (
          step.command.startsWith('FLUSHALL') &&
  ...
  const handleNext = () => {
    ...
    if (currentStepIndex < currentModule.steps.length - 1) {
      ...
      const currIdx = CURRICULUM.modules.findIndex(
    ...
    <div className='flex flex-col lg:flex-row h-screen lg:h-[100dvh] bg-robot-dark text-slate-200 font-sans overflow-hidden'>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            <div
              className='flex-1 overflow-y-auto rounded-3xl bg-black/20 border border-white/5 p-4 pb-2 custom-scrollbar'
              ...
              <div className='space-y-2'>
                {CURRICULUM.modules.map((module, idx) => {
                  ...
                  const isLocked =
                    ...
                    !isCurrent &&
                    idx > 0 &&
                    !completedModules.includes(CURRICULUM.modules[idx - 1].id)
                      ...
                      <div className='flex items-center justify-between mb-1 relative z-10'>
                        <span
                          className={`text-[10px] uppercase tracking-wider font-bold ${
                            isCurrent ? 'text-redis-400' : 'text-slate-500'
                          }`}
                        >
                          Module {idx + 1}
                        </span>
      ...
      <main className='flex-1 flex flex-col min-h-0 overflow-hidden relative bg-[#0B1221] lg:bg-[#0B1221]'>
        ...
        <div
          className={`flex-1 flex flex-col lg:flex-row p-2 pt-[88px] lg:p-4 lg:pt-4 gap-2 lg:gap-4 min-h-0 relative transition-all duration-200 ${
            isKeyboardOpen
              ...
              : 'pb-[120px] lg:pb-4 overflow-hidden'
          }`}
        ...
        >
          ...
          <div
            className={`flex flex-col gap-2 lg:gap-4 w-full lg:w-1/2 h-full min-h-0 ${
              isKeyboardOpen
                ? 'overflow-visible'
                : 'overflow-y-auto lg:overflow-visible'
            } ${mobileTab === 'visualizer' ? 'hidden lg:flex' : 'flex'}`}
          >
            ...
            <div
              className={`relative z-0 transition-all duration-200 ${
                isKeyboardOpen
                  ? 'fixed inset-x-2 top-[88px] bottom-0 flex flex-col lg:relative lg:inset-auto lg:flex-1'
                  : 'flex-1 min-h-[200px]'
              }`}
            >
              <Terminal
                ...
                suggestedCommand={
                  currentModule.steps[currentStepIndex].input_code
                }
        ...
        <div
          className={`lg:hidden fixed bottom-6 inset-x-4 z-30 pointer-events-none transition-all duration-200 ${
            isKeyboardOpen
        ...
        >
          {/* 
       Changes made:
       1. Changed 'z-50' to 'z-30' in the parent div above. 
          (This allows the Sidebar to slide OVER this menu).
    */}
          ...
          <div className='pointer-events-auto mx-auto max-w-sm bg-[#0f172a]/95 backdrop-blur-2xl border border-slate-700/50 rounded-3xl p-1.5 shadow-[0_8px_32px_rgba(0,0,0,0.5)] flex relative ring-1 ring-white/10 overflow-hidden'>
            ...
            >
              <TerminalIcon
                className={`w-4 h-4 ${
                  mobileTab === 'terminal' ? 'stroke-[2.5px]' : 'stroke-2'
                }`}
              />
              <span>TERMINAL</span>
            </button>
            ...
            >
              <Cpu
                className={`w-4 h-4 ${
                  mobileTab === 'visualizer' ? 'stroke-[2.5px]' : 'stroke-2'
                }`}
              />
