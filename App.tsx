...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-[100dvh] bg-robot-dark text-slate-200 font-sans overflow-hidden">
      ...
      {/* UNIQUE MOBILE HEADER: Floating HUD Style */}
      <header className="md:hidden fixed top-0 inset-x-0 z-50 p-4 pointer-events-none">
      ...
      {mobileMenuOpen && (
        <motion.div 
            ...
            className="fixed inset-0 z-40 bg-slate-900/95 backdrop-blur-xl md:hidden flex flex-col pt-24 pb-8 px-4"
        >
             ...
             <div className="mt-4 flex justify-center gap-6 p-4 rounded-2xl bg-black/20 border border-white/5">
                ...
                <a href="#" className="text-slate-500 hover:text-[#0077b5]"><Linkedin className="w-5 h-5" /></a>
                <a href="#" className="text-slate-500 hover:text-emerald-400"><Globe className="w-5 h-5" /></a>
             </div>
        </motion.div>
      )}
      </AnimatePresence>
      ...
      <main className="flex-1 flex flex-col min-h-0 overflow-hidden relative bg-[#0B1221] md:bg-[#0B1221]">
         
         {/* DESKTOP STATUS BAR - Matches Mobile HUD Aesthetic */}
         <div className="hidden md:flex items-center justify-between px-6 py-4 border-b border-white/5 bg-[#0f172a]/50 shrink-0">
             ...
             {/* Latency component removed */}
         </div>

         {/* Main Content Area - Added top padding for HUD header on mobile, normal on desktop */}
         <div className="flex-1 flex flex-col md:flex-row p-2 pt-24 pb-28 md:p-6 md:pb-6 gap-2 md:gap-6 min-h-0 overflow-hidden relative">
         ...
         {/* UNIQUE BOTTOM NAV: Floating Control Deck */}
         <div className="md:hidden fixed bottom-6 inset-x-4 z-50 pointer-events-none">
             <div className="pointer-events-auto mx-auto max-w-sm bg-[#0f172a]/95 backdrop-blur-2xl border border-slate-700/50 rounded-3xl p-1.5 shadow-[0_8px_32px_rgba(0,0,0,0.5)] flex relative ring-1 ring-white/10 overflow-hidden">
                 ...
                 <div 
                    className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-gradient-to-b from-redis-600 to-redis-700 rounded-2xl shadow-lg shadow-redis-900/50 transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                        mobileTab === 'terminal' ? 'left-1.5' : 'left-[calc(50%+3px)]'
                    }`} 
                 >
                    {/* Top shine */}
