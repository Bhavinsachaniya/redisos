...
const Sidebar: React.FC<SidebarProps> = ({ modules, currentModuleId, completedModules, currentStepIndex, onSelectModule, onSelectStep }) => {
  return (
    <div className="w-full lg:w-64 bg-[#0f172a]/95 backdrop-blur-xl border-r border-white/5 flex flex-col hidden lg:flex h-full shadow-[4px_0_24px_rgba(0,0,0,0.4)] z-20 relative">
      {/* Decorative tech lines */}
      <div className="absolute top-0 right-0 w-[1px] h-24 bg-gradient-to-b from-redis-500/50 to-transparent opacity-50" />
      
      {/* Sidebar Header */}
      <div className="p-5 pb-5 border-b border-white/5 shrink-0 relative overflow-hidden">
          ...
          return (
            <div key={module.id} className="group/item">
              ...
              >
                ...
                <h3 className="text-sm font-semibold relative z-10 leading-snug">
                  {module.title}
                </h3>
              </button>
                    ...
                    {module.steps.map((step, sIdx) => {
                        ...
                        return (
                            <button 
                                ...
                                className={`
                                w-full text-left py-1.5 px-2 rounded-lg text-xs font-mono transition-colors flex items-center gap-2
                                ${isStepCurrent ? 'bg-white/5 text-white' : isStepCompleted ? 'text-slate-500 hover:text-slate-300' : 'text-slate-600 hover:text-slate-400'}
                            `}>
                                {isStepCurrent ? (
