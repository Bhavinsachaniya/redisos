...
const HistoryItem: React.FC<{
  entry: { command: string; output: string; status: 'success' | 'error' }
}> = ({ entry }) => {
  ...
  return (
    <div className='space-y-1'>
      <div className='flex items-center gap-2 text-slate-300 text-xs md:text-sm'>
        <span className='text-redis-500 shrink-0 text-xs md:text-sm'>
          127.0.0.1:6379&gt;
        </span>
      ...
      <div className='relative group'>
        <motion.div
          ...
          } whitespace-pre-wrap pl-2 md:pl-3 border-l-2 border-slate-700/50 pr-10 md:pr-8 text-xs md:text-sm leading-relaxed break-words overflow-x-auto`}
        >
          {entry.output}
        </motion.div>
...
const Terminal: React.FC<TerminalProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      suggestedCommand &&
      suggestedCommand.toLowerCase().startsWith(trimmed.toLowerCase())
    ) {
  ...
  const isMatchingSuggestion =
    suggestedCommand &&
    input.trim() &&
    suggestedCommand.toLowerCase().startsWith(input.trim().toLowerCase())

  return (
    <div
      ref={terminalRef}
      className={`flex flex-col h-full bg-black/90 rounded-xl overflow-hidden border border-slate-700 shadow-2xl font-mono relative mb-10 ${
        isKeyboardOpen ? 'lg:h-3/4' : ''
      }`}
      ...
      <div className='relative shrink-0'>
          ...
          {hint && input.trim() && !isKeyboardOpen && (
            ...
            >
              {isMatchingSuggestion ? (
                <>
                  <span className='text-yellow-400 font-bold shrink-0 text-[9px] md:text-xs'>
                    Suggestion:
                  </span>
                ...
                <>
                  <span className='text-slate-400 font-bold shrink-0 text-[9px] md:text-xs'>
                    Usage:
                  </span>
                  <span className='text-redis-400 font-bold font-mono truncate'>
                    {input.trim().split(' ')[0].toUpperCase()}
                  </span>
                  <span className='text-slate-500 font-mono truncate min-w-0'>
                    {hint}
                  </span>
                </>
              )}
        ...
        <form
          ...
          className={`p-3 md:p-4 bg-slate-900 border-t border-slate-800 flex items-center gap-2 z-50 ${
            isKeyboardOpen
              ? 'fixed bottom-0 left-2 right-2 lg:relative lg:bottom-auto shadow-[0_-8px_24px_rgba(0,0,0,0.8)] rounded-b-xl pb-safe'
              : 'relative'
          }`}
          style={
            isKeyboardOpen && window.innerWidth < 1024
              ? {
        ...
        >
          <span className='text-redis-500 font-bold shrink-0 text-sm md:text-base'>
            127.0.0.1:6379&gt;
          </span>
          <input
            ref={inputRef}
          ...
          <button
            type='submit'
            className={`p-2 rounded-md transition-colors shrink-0 touch-manipulation min-h-[44px] min-w-[44px] flex items-center justify-center ${
              input.trim()
                ? 'bg-redis-600 text-white hover:bg-redis-500 active:bg-redis-700'
